...
class MainWindow(QMainWindow):
    ...
    def Open(self, full_paths):
        if full_paths == False:
            full_paths = 
            QFileDialog.getOpenFileNames(self, 'Open File')[0]
...
class UI(QWidget):
    ...
    def create_slider(self, path):
        ...
        slider.setStyleSheet(
            'QSlider::handle:vertical {background-color: '+ hex_color + ';}')
        ...
        name = os.path.basename(path).replace('.xlsx', r"")
        button.setText(name)
    ...
    def update_graph(self):  # use animation
        ...
        self.ax.bar(xs, self.soilValues, color=r"grey")
        ...
        for index in range(self.sliders.count()):
            ...
            self.ax.bar(xs, ys, bottom=values_sum,
                        color=self.colors[index], edgecolor='black')
            values_sum += ys
            T_Ha = round(1330 * slider_value / self.sliderTicks, 1)
            self.values.itemAt(index).widget().setText(str(T_Ha)
                    + 'T/Ha')

        self.ax.bar(xs, self.IdealValues, facecolor='None',
                    edgecolor='green')  # want out of frame
        self.ax.bar(xs, self.IdealValues * 4, facecolor='None',
                    edgecolor='red')  # want out of frame
